...
jQuery.extend(jQuery.easing, {
    ...
    easeInOutBack: function(e, f, a, i, h, g) {
        return void 0 == g && (g = 1.70158),
        (f /= h / 2) < 1 ? i / 2 * f * f * (((g *= 1.525) + 1) * f - g) + a: i / 2 * ((f -= 2) * f * (((g *= 1.525) + 1) * f + g) + 2) + a
    },
...
function($) {
    ...
    turnMethods = {
        init: function(opts) {
            ...
            return this.turn("page", opts.page),
            corners = $.extend({},
        ...
        addPage: function(element, page) {
            ...
            incPages = !0;
            return page >= 1 && lastPage >= page && (data.done && this.turn("stop"), page in data.pageObjs && turnMethods._movePages.call(this, page, 1), incPages && (data.totalPages = lastPage), data.pageObjs[page] = $(element).addClass("turn-page p" + page), turnMethods._addPage.call(this, page), data.done && this.turn("update"), turnMethods._removeFromDOM.call(this)),
            this
        },
        _addPage: function(page) {
            ...
            if (element) if (turnMethods._necessPage.call(this, page)) {
                if (!data.pageWrap[page]) {
                    ...
                    }).css(pagePosition["double" == data.display ? page % 2 : 0]),
                    this.append(data.pageWrap[page]),
                    data.pageWrap[page].prepend(data.pageObjs[page])
                }
                page && 1 != turnMethods._setPageLoc.call(this, page) || turnMethods._makeFlip.call(this, page)
            } else data.pagePlace[page] = 0,
            data.pageObjs[page] && data.pageObjs[page].remove()
        },
        ...
        range: function(page) {
            ...
            return view[1] = view[1] || view[0],
            view[0] >= 1 && view[1] <= data.totalPages ? (remainingPages = Math.floor((pagesInDOM - 2) / 2), data.totalPages - view[1] > view[0] ? (left = Math.min(view[0] - 1, remainingPages), right = 2 * remainingPages - left) : (right = Math.min(data.totalPages - view[1], remainingPages), left = 2 * remainingPages - right)) : (left = pagesInDOM - 1, right = pagesInDOM - 1),
            [Math.max(1, view[0] - left), Math.min(data.totalPages, view[1] + right)]
        },
        ...
        _removePageFromDOM: function(page) {
            ...
            if (data.pages[page]) {
                ...
                dd.f && dd.f.fwrapper && dd.f.fwrapper.remove(),
                data.pages[page].remove(),
                delete data.pages[page]
            }
            data.pageObjs[page] && data.pageObjs[page].remove(),
            data.pageWrap[page] && (data.pageWrap[page].remove(), delete data.pageWrap[page]),
            delete data.pagePlace[page]
        },
        removePage: function(page) {
            var data = this.data();
            return data.pageObjs[page] && (this.turn("stop"), turnMethods._removePageFromDOM.call(this, page), delete data.pageObjs[page], turnMethods._movePages.call(this, page, -1), data.totalPages = data.totalPages - 1, turnMethods._makeRange.call(this), data.page > data.totalPages && this.turn("page", data.totalPages)),
            this
        },
        _movePages: function(from, change) {
            ...
            move = function(page) {
                ...
                odd = next % 2;
                data.pageObjs[page] && (data.pageObjs[next] = data.pageObjs[page].removeClass("page" + page).addClass("page" + next)),
                data.pagePlace[page] && data.pageWrap[page] && (data.pagePlace[next] = next, data.pageWrap[next] = data.pageWrap[page].css(pagePosition[single ? 0 : odd]).attr("page", next), data.pages[page] && (data.pages[next] = data.pages[page].flip("options", {
                    page: next,
        ...
        _removeMv: function(page) {
            var i, data = this.data();
            for (i = 0; i < data.pageMv.length; i++) if (data.pageMv[i] == page) return data.pageMv.splice(i, 1),
            !0;
            return ! 1
        },
        ...
        _view: function(page) {
            ...
            "double" == data.display ? page % 2 ? [page - 1, page] : [page, page + 1] : [page]
        },
        view: function(page) {
            var data = this.data(),
        ...
        stop: function() {
            ...
            pages = data.pageMv;
            data.pageMv = [],
            data.tpage && (data.page = data.tpage, delete data.tpage);
        ...
        _fitPage: function(page, ok) {
            ...
            data.page != page && (this.trigger("turning", [page, newView]), -1 != $.inArray(1, newView) && this.trigger("first"), -1 != $.inArray(data.totalPages, newView) && this.trigger("last")),
            data.pageObjs[page] && (data.tpage = page, this.turn("stop", ok), turnMethods._removeFromDOM.call(this), turnMethods._makeRange.call(this), this.trigger("turned", [page, newView]))
        },
        _turnPage: function(page) {
            ...
            if (data.page != page && (this.trigger("turning", [page, newView]), -1 != $.inArray(1, newView) && this.trigger("first"), -1 != $.inArray(data.totalPages, newView) && this.trigger("last")), data.pageObjs[page] && (data.tpage = page, this.turn("stop"), turnMethods._makeRange.call(this), "single" == data.display ? (current = view[0], next = newView[0]) : view[1] && page > view[1] ? (current = view[1], next = newView[0]) : view[0] && page < view[0] && (current = view[0], next = newView[1]), data.pages[current])) {
                ...
                data.tpage = next,
                opts.next != next && (opts.next = next, data.pagePlace[next] = opts.page, opts.force = !0),
                "single" == data.display ? data.pages[current].flip("turnPage", newView[0] > view[0] ? "br": "bl") : data.pages[current].flip("turnPage")
            }
        },
        page: function(page) {
        ...
        _end: function(e, turned) {
            ...
            dd = turn.data();
            e.stopPropagation(),
            turned || dd.tpage ? (dd.tpage == opts.next || dd.tpage == opts.page) && (delete dd.tpage, turnMethods._fitPage.call(turn, dd.tpage || opts.next, !0)) : (turnMethods._removeMv.call(turn, opts.pageMv), turn.turn("update"))
        },
        _pressed: function() {
            var page, that = $(this),
            data = that.data().f,
        ...
        _released: function(e, point) {
            ...
            data = that.data().f;
            e.stopPropagation(),
            ((new Date).getTime() - data.time < 200 || point.x < 0 || point.x > $(this).width()) && (e.preventDefault(), data.opts.turn.data().tpage = data.opts.next, data.opts.turn.turn("update"), $(that).flip("turnPage"))
        },
        _flip: function() {
            var opts = $(this).data().f.opts;
        ...
        calculateZ: function(mv) {
            ...
            addView = function(page) {
                ...
                view[0] && (r.pageV[view[0]] = !0),
                view[1] && (r.pageV[view[1]] = !0)
            };
            for (i = 0; i < mv.length; i++) page = mv[i],
            ...
            r.partZ[placePage] = 2 * data.totalPages + Math.abs(currentPage - dpage);
            return r
        },
        update: function() {
            var page, data = this.data();
    ...
    flipMethods = {
        init: function(opts) {
            return opts.gradients && (opts.frontGradient = !0, opts.backGradient = !0),
            this.data({
                f: {}
            }),
        ...
        _addPageWrapper: function() {
            ...
            if (!data.wrapper) {
                ...
                if (data.parent = parent, data.fparent = data.opts.turn ? data.opts.turn.data().fparent: $("#turn-fwrappers"), !data.fparent) {
                    ...
                    data.opts.turn ? (fparent.css(divAtt( - data.opts.turn.offset().top, -data.opts.turn.offset().left, "auto", "visible").css).appendTo(data.opts.turn), data.opts.turn.data().fparent = fparent) : fparent.css(divAtt(0, 0, "auto", "visible").css).attr("id", "turn-fwrappers").appendTo($("body")),
                ...
                data.opts.frontGradient && (data.ashadow = $("<div/>", divAtt(0, 0, 1)).appendTo(data.fpage)),
                flipMethods.setData.call(this, data),
                flipMethods.resize.call(this, !0)
            }
        },
        _fold: function(point) {
            ...
            compute = function() {
                ...
                if (tr = point2D(distance * Math.sin(alpha), distance * Math.cos(alpha)), alpha > A90 && (tr.x = tr.x + Math.abs(tr.y * Math.tan(tan)), tr.y = 0, Math.round(tr.x * Math.tan(PI - alpha)) < height)) return point.y = Math.sqrt(Math.pow(height, 2) + 2 * middle.x * rel.x),
                ...
                return gradientOpacity = width > far ? far / width: 1,
                data.opts.frontGradient && (gradientStartV = gradientSize > 100 ? (gradientSize - 100) / gradientSize: 0, gradientEndPointA = point2D(gradientSize * Math.sin(A90 - alpha) / height * 100, gradientSize * Math.cos(A90 - alpha) / width * 100), top && (gradientEndPointA.y = 100 - gradientEndPointA.y), left && (gradientEndPointA.x = 100 - gradientEndPointA.x)),
                flipMethods._backGradient.call(that) && (gradientEndPointB = point2D(gradientSize * Math.sin(alpha) / width * 100, gradientSize * Math.cos(alpha) / height * 100), left || (gradientEndPointB.x = 100 - gradientEndPointB.x), top || (gradientEndPointB.y = 100 - gradientEndPointB.y)),
            ...
            transform = function(tr, c, x, a) {
                ...
                data.opts.frontGradient && gradient(data.ashadow, point2D(left ? 100 : 0, top ? 100 : 0), point2D(gradientEndPointA.x, gradientEndPointA.y), [[gradientStartV, "rgba(0,0,0,0)"], [.8 * (1 - gradientStartV) + gradientStartV, "rgba(0,0,0," + .2 * gradientOpacity + ")"], [1, "rgba(255,255,255," + .2 * gradientOpacity + ")"]], 3, alpha),
                flipMethods._backGradient.call(that) && gradient(data.bshadow, point2D(left ? 0 : 100, top ? 0 : 100), point2D(gradientEndPointB.x, gradientEndPointB.y), [[.8, "rgba(0,0,0,0)"], [1, "rgba(0,0,0," + .3 * gradientOpacity + ")"], [1, "rgba(0,0,0,0)"]], 3)
            };
            switch (point.corner) {
            case "tl":
                ...
                data.fpage.transform(translate( - height, -width, ac) + rotate(90 - 2 * a), "100% 100%"),
            ...
            case "tr":
                ...
                data.fpage.transform(translate(0, -width, ac) + rotate( - 90 + 2 * a), "0% 100%"),
            ...
            case "bl":
                ...
                data.fpage.transform(translate( - height, 0, ac) + rotate( - 90 + 2 * a), "100% 0%"),
                folding.transform(rotate(270) + translate( - width, 0, ac), "0% 0%");
                break;
            case "br":
                point.x = Math.min(point.x, width - 1),
                ...
                data.fpage.transform(rotate(90 - 2 * a), "0% 0%"),
                folding.transform(rotate(90) + translate(0, -height, ac), "0% 0%")
            }
            data.point = point
        },
        ...
        _showFoldedPage: function(c, animate) {
            ...
            if (folding) {
                ...
                } else flipMethods._fold.call(this, c),
                dd.effect && !dd.effect.turning && this.animatef(!1);
                return data.fwrapper.is(":visible") || (data.fparent.show().data().flips++, flipMethods._moveFoldingPage.call(this, !0), data.fwrapper.show(), data.bshadow && data.bshadow.show()),
                !0
            }
            return ! 1
        },
        hide: function() {
            var data = this.data().f,
            folding = flipMethods._foldingPage.call(this);
            return 0 === --data.fparent.data().flips && data.fparent.hide(),
            this.css({
                left: 0,
                top: 0,
                right: "auto",
            ...
            data.fwrapper.hide(),
            data.bshadow && data.bshadow.hide(),
        ...
        _eventStart: function(e) {
            ...
            if (!data.disabled && !this.flip("isTurning")) {
                if (data.corner = flipMethods._cornerActivated.call(this, e), data.corner && flipMethods._foldingPage.call(this, data.corner)) return flipMethods._moveFoldingPage.call(this, !0),
        ...
        _eventMove: function(e) {
            ...
            if (!data.disabled) if (e = isTouch ? e.originalEvent.touches: [e], data.corner) {
                ...
                if (corner) {
                    ...
                    corner.x = origin.x,
                    corner.y = origin.y,
                    flipMethods._showFoldedPage.call(this, corner, !0)
        ...
        _eventEnd: function() {
            ...
            if (!data.disabled && data.point) {
                ...
                event.isDefaultPrevented() || flipMethods.hideFoldedPage.call(this, !0)
            }
            data.corner = null
        },
    ...
    $.extend($.fn, {
        ...
        transform: function(transform, origin) {
            var properties = {};
            return origin && (properties[vendor + "transform-origin"] = origin),
        ...
        animatef: function(point) {
            ...
            if (data.effect && clearInterval(data.effect.handle), point) {
                point.to.length || (point.to = [point.to]),
                point.from.length || (point.from = [point.from]),
                point.easing || (point.easing = function(x, t, b, c, data) {
                ...
                f = function() {
                    ...
                    point.frame(1 == len ? v[0] : v),
                    time == point.duration && (clearInterval(data.effect.handle), delete data.effect, that.data(data), point.complete && point.complete())
                };
...
function(window, jq) {
    function checkSize() {
        ...
        calcByWidth = 1900 / 1084 > winWidth / winHeight,
        601 > winHeight && (jq("body").addClass("m"), noAnimate = !0)
    }
    function onResize() {
        checkSize(),
        1366 > winWidth ? jq("#a .info-wrap").addClass("info-wrap-sort") : jq("#a .info-wrap").removeClass("info-wrap-sort"),
        1e3 > winWidth ? jq("body").width(1e3) : jq("body").css("width", "100%");
        var screenHeight = winHeight - getTopHeight() - getFooterHeight();
        return jq(".screen").height(screenHeight),
        scrollSize = screenHeight,
        jq("#fullPage").height(winHeight - getTopHeight()),
        jq("#scroller").css("top", curIndex * screenHeight * -1),
        is_mobi && window.location.search.indexOf("mv=fp") < 0 ? void jq("head").append("<link rel='stylesheet' type='text/css' href='css/ie-fixed.css' />") : (resizeA(screenHeight), void resizeObj(screenHeight))
    }
    ...
    function sideNavFunc() {
        jq(".side-nav-item").each(function() {
            ...
            function() {
                scrollTo(index),
                $sideNav.removeClass("side-nav-0 side-nav-1 side-nav-2 side-nav-3 side-nav-4 side-nav-5"),
                $sideNav.addClass("side-nav-" + index),
                jq("#sideNav .side-nav-item").removeClass("side-nav-item-cur")
            })
        })
    }
    ...
    function scrollTo(targetIndex) {
        if (! (0 > targetIndex && targetIndex > 5)) {
            ...
            function() {
                ...
                onAfterScroll(direction)
            })
        }
    }
    function onBeforeScroll() {
        0 === curIndex && jq(".info-wrap").hide(),
        (0 === curIndex || 5 === curIndex) && jq("#footer").hide(),
        2 === curIndex && 2 == jq("#booksTurn").turn("page") && jq("#booksTurn").turn("page", 1),
    ...
    function onAfterScroll() {
        ...
        $sideNav.removeClass("side-nav-0 side-nav-1 side-nav-2 side-nav-3 side-nav-4 side-nav-5"),
        $sideNavitem.eq(curr).addClass("side-nav-item-cur"),
        $sideNav.addClass("side-nav-" + curr),
        0 === curIndex ? (jq(".info-wrap").fadeIn(), jq("#fullPage").addClass("show-a")) : jq("#fullPage").removeClass("show-a"),
        (5 === curIndex || 0 === curIndex) && jq("#footer").fadeIn(),
        2 === curIndex && setTimeout(function() {
        ...
        2e3)),
        5 !== curIndex && jq(".arrow-down").show(),
        0 !== curIndex && jq(".arrow-up").show()
    }
